...
|--------                |---------                                     |---------                  |------------
| `echo`                 |                                              | `true`                    | Subscriptions receive messages published by the client. Requires server support (1.2.0). If set to false, and the server doesn't support the feature, an error with code `NATS_PROTOCOL_ERR` is emitted, and the connection is aborted. Note that it is possible for this error to be emitted on reconnect when the server reconnects to a server that does not support the feature.
